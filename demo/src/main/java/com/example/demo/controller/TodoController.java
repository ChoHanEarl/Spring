...
//@Controller + @ResponseBody => @RestController 직렬화 해서 반환
@RequestMapping("todo")
...
//			String temporaryUserId = "temporary-user"; // 임시 유저 id
			...
			//TodoDTO -> Entity로 바꿔서 DB에 저장
			//-------------------------------------------------------------
			//Entity를 담은 List -> TodoDTO -> response
			
			//자바 스트림을 이용해 반환된 엔티티리스트를 TodoDTO객체를 담은 리스트로 반환한다.
			//response를 내보낼 때 TodoDTO도 내보내기로 했으니 TodoDTO
			...
			return ResponseEntity.ok().body(response);  //responseDTO객체를 돌려줘야 함
		} catch (Exception e) {
			//혹시 예외가 발생하는 경우 responseDTO객체에 리스트 대신 error메시지를 넣어 반환한다.
...
//		String temporaryUserId = "temporary-user";
		...
		//서비스레이어의 retrieve메서드를 이용해 TodoEntity가 담겨 있는 리스트를 반환받아 entities에 저장한다.
...
//			String temporaryUserId = "temporary-user"; //임시 유저 아이디
			...
			//4. 자바 스트림을 이용해 반환된 엔티티 리스트를 TodoDTO리스트로 변환
			List<TodoDTO> dtos = entities.stream().map(TodoDTO::new).collect(Collectors.toList());
