...
@Configuration
@EnableWebSecurity //웹 보안을 활성화 하는 데 사용한다.
//스프링 시큐리티 설정을 활성화
public class WebSecurityConfig {
	...
	protected DefaultSecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
		...
		//빌더패턴을 사용해서 인증, 권한부여, CSRF, 세션관리 등등을 처리할 수 있다.
		http
			//WebMvcConfig에서 cors설정을 했으므로 이곳(Security)에서는 비활성화
			...
			authorizeRequestConfigurer.requestMatchers("/", "/auth/**").permitAll() // 특정 경로는 인증없이 허용
			.anyRequest().authenticated() // 나머지 요청에 대해서는 인증 필요
			);
		
		// filter 등록
		// CorsFilter를 실행한 후 jwtAuthenticationFilter를 실행하겠다.
