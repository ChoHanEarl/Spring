...
public class JwtAuthenticationFilter extends OncePerRequestFilter{
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		// 필터에서 뭘 하고싶은지 작성(토큰을 가지고 유효한지 검증)
		try {
			//1. 발급된 토큰 읽어오기
			//parseBearerToken() : request에 담긴 토큰을 꺼내는 메서드
			...
			if(token != null && token.equalsIgnoreCase("null")) {
				...
				//request로부터 인증 세부 정보를 생성하는 역할을 한다.
				
				//.buildDetails(request)
				//request객체에서 인증과 관련된 추가적인 정보를 추출한다.
				//사용자의 세션 ID, 클라이언트의 IP 주소 등의 메타데이터를 포함한다.
				authentication.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
				
				//SecurityContext
				//인증된 정보를 저장
				//SecurityContextHolder
				//스프링 시큐리티에서 사용자의 인증 정보와 Context를 관리하는 중심 클래스
